...
def batch_compute_model_accuracy(predictor, output_labels, dataset, batch_size=1000, get_adv_accuracy=False):
        ...
        for element in data_test:
            ...
            for result in output:
                ...
                if(ret[0] == labels_batch[data_element_batch]):
                    ...
                    wpi_i.append(img)                
                    # EDITED: Aug 23, 
                    #       commented line list.append(wrong_labels, labels_batch[data_element_batch]) and added list.append(wrong_labels, ret[0]).
                    #       This was done so that the incorrectly predicted images have label which they were predicted against, not their original labels
                    list.append(wrong_labels, ret[0])
    ...
    avgl2 = 0
    if (get_adv_accuracy):
        avgl2, avglinf = get_lp_norm_distances(wpi_i, image_index_incorrectly_predicted)
...
def save_files_to_disc(cpi_i, correct_labels, wpi_i, wrong_labels, image_index_correctly_predicted, image_index_incorrectly_predicted, get_adv_accuracy):    
    ...
    save_correct_pred = save_or_load_image__npz(
            "save", 
    ...
    save_false_pred = save_or_load_image__npz(
            "save", 
