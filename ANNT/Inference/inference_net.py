...
from  .inference_core import infer_model, show_one_predictions, infer_model_dataset, multiple_infer_model_dataset
...
def get_dataset(inference_image_source):
    ...
    if(inference_image_source.lower() == "mnist"):
        ...
        """
        data_test = save_or_load_image__npz("load", inference_image_source)
    return data_test
...
def initiate_inference(userconfig):
    ...
    inference_image_source = userconfig["inference-options"]["dataset"]
    ...
    #get the set of images specified by user for inference
    inference_data = userconfig["inference-options"]["images"]
    
    #setup logger
    dataset_name_log = inference_image_source
    #set log directory to the current inference instance based on the inference set provided and not generic dataset name
    if(inference_data is not None and len(inference_data)>0):
        dataset_name_log = inference_image_source + "_" + os.path.basename(inference_data)
        
    if(precision is None):
        log_name = "FP"
    else:
        log_name = precision    
    logger.set_logger_dir(os.path.join('logs','inference_log', '{}-{}'.format(dataset_name_log, log_name)), action="d")
    ...
    if(inference_image_source == "mnist"):
        model = get_model(get_mod_typ(inference_model_description, precision), precision)
    elif(inference_image_source == "cifar10"):
        model = get_model_cifar(get_mod_typ_cifar(inference_model_description, precision), precision)
    
    #when getting the dataset for inference, use the image set if the user provides it, otherwise use the test split of the data set in which model was trained.
       
    if (inference_data is None or len(inference_data)<=0):
        inference_data = inference_image_source
    ...
    data_test = get_dataset(inference_data)
    ...
    accuracy_t1, error_t1, saved_corr_images, saved_false_images, _, _ = inference_mode(predictorConfig, data_test)
