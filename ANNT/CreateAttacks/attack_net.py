...
#from ..DataSets import dataset_location_manager  as loc_mng
from ..DataSets.mnist import GetMnist
from .supported_attacks import SupportedAlgorithms
from .create_adversarial_samples import get_adversarial_samples
from ..save_restore_images import * 
...
random_sampling = True                  #select True to randomly select images from dataset

def create_attack_mode(config, algorithm,  precision, data_test, source_data, randomize, sample_limit, total_iterations, prefiltered, targets, attack_params):
    """
    ...
    randomize: True if samples should be randomly selected from the dataset, when false the application will sequentially pick samples.

    sample_limit: how many examples to create

    total_iterations: how many times the application should generate the given number of samples (each iteration generates "sample_limit" number of samples). 
    NOTE: SET ITERATIONS 1 WHEN RANDOMIZE IF FALSE because it does not make sense to generate the same samples multiple times.

    targets: list of target labels. a list
    
    attack_parameters: attack specific hyper-parameters

    """
    
    random_sampling = randomize
    Adv_sample_number_Src = sample_limit
    ...
    if(prefiltered):            
        ...
        #if prefiltered is off then the data is just a sequential set of images so give image index accordingly
        image_index = get_image_index(data_test)
    ...
    for i in range(iterations):
        if(random_sampling):
            ...
            craft_labels = source_data.labels[random_index]
            craft_image_index = random_index
        else:
            craft_examples = source_data.images[:Adv_sample_number_Src]
            craft_labels = source_data.labels[:Adv_sample_number_Src]
            craft_image_index = image_index[:Adv_sample_number_Src]
        
        # craft adversarial attacks
        adv_samples = get_adversarial_samples(config, craft_examples, algorithm, targets, attack_params)
...
def get_image_index(data_samples):
    ...
    #0 based index
    logger.info(type(data_samples))
    return(np.arange(0, data_samples.images.shape[0]+1, 1)) 
...
def get_attack_algorithm(algorithm_str):
    ...
    elif(algorithm_str.lower() == "cw_l0"):
         supported_alg = SupportedAlgorithms.CW_l0
    elif(algorithm_str.lower() == "cw_l1"):
         supported_alg = SupportedAlgorithms.CW_l1
    elif(algorithm_str.lower() == "cw_l2"):
        supported_alg = SupportedAlgorithms.CW_L2
    else:
...
def get_data(dataset_name):
    ...
    """
    #dataset_loc = loc_mng.dataset_manager.get_dataset_directory()
    if (dataset_name.lower() == "mnist"):        
        data_test = GetMnist('test', shuffle=False, dir=Mnist_Data_Dir)
        #data_test = GetMnist('test', shuffle=False, dir=dataset_loc)
    elif (dataset_name.lower() == "cifar10"):
        data_test = get_cifar10_data('test', dir= Cifar_Data_Dir)
        #data_test = get_cifar10_data('test', dir= dataset_loc)
    else:
...
def get_prefiltered_images(saved_corr_images, adv_images):
    ...
    adv_data = save_or_load_image__npz("load", adv_images)
    ...
    for img_data in saved_corr_images: 
        ...
        if(len(image_index) >= Adv_sample_number_total):
            break
    savedir = logger.get_logger_dir() 
    save_prefiltered_images = save_or_load_image__npz(
        "save", 
...
def create_attack(userconfig):
    ...
    if(precision is None):
        ...
        log_name = precision    
    logger.set_logger_dir(os.path.join('logs', 'trained_images', '{}-{}'.format(dataset_name_log, log_name)), action="d")
        
    #get model architecture
    #model = mr.get_model(mr.get_mod_typ(attack_model_description, precision), precision)
    
    if(userdataset == "mnist"):
        model = get_model(get_mod_typ(attack_model_description, precision), precision)
    elif(userdataset == "cifar10"):
        model = get_model_cifar(get_mod_typ_cifar(attack_model_description, precision), precision)

    #setup configurations for restoring model for predictions and for creating adversarial attacks
    config = inference_net.get_prediction_config(saved_model, model)

    attacks_params = userconfig["attack-options"]["create-attack"]["parameters"]

    randomize = userconfig["attack-options"]["create-attack"]["randomize"]

    sample_limit = userconfig["attack-options"]["create-attack"]["sample-limit"]

    iterations = userconfig["attack-options"]["create-attack"]["iterations"]

    prefiltered = userconfig["attack-options"]["create-attack"]["prefiltered"]

    if(prefiltered):
        #get data that are correctly classified by the classifier 
        _, _, saved_corr_images,_, _, _ =  inference_mode(config, dataset)
    else:
        saved_corr_images = dataset
        
    #in case of targetted attacks, get the list of target labels
    targets = userconfig["attack-options"]["create-attack"]["targets"]
    
    #create adversarial attacks for the specified model
    adv_samples = create_attack_mode(config, algorithm, log_name, saved_corr_images, dataset, randomize, sample_limit, iterations, prefiltered, targets, attacks_params)
    return adv_samples
...
def transfer_attack(userconfig):
    ...
    logger.set_logger_dir(os.path.join('logs','transfer_log','from-{}-transfer-'.format(source_log_name), '{}-{}'.format(dataset_name_log, log_name)), action="d")
...
def initiate_attack_transfer(userdataset, target_model_description, target_precision, target_saved_model, source_adv_images):
    ...
    if(userdataset == "mnist"):
        model = get_model(get_mod_typ(target_model_description, target_precision), target_precision)
    elif(userdataset == "cifar10"):
        model = get_model_cifar(get_mod_typ_cifar(target_model_description, target_precision), target_precision)
        
    #setup configurations for restoring model for predictions and for creating adversarial attacks
    config = inference_net.get_prediction_config(target_saved_model, model)

    #get data that are correctly classified by the classifier 
    _, _, saved_corr_images, saved_adv_false_images, _, _ =  inference_mode(config, dataset)
    ...
    adv_acc, transfer_rate, saved_adv_corr_images, saved_adv_false_images, agvl2, avglinf =  inference_mode(config, pre_filtered_images, True)
...
def initiate_attack_creation(userconfig):
    ...
    if(userconfig["attack-options"]["attack-mode"].lower() == "create"):
        adv_samples = create_attack(userconfig)
        return adv_samples
