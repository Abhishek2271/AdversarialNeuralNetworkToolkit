...
def get_config_mnist(_model, data_train, data_test, epochs):
    """
...
def get_config_cifar(_model, data_train, data_test, epochs):
...
def get_config_cifar_resnet(_model, data_train, data_test, epochs):
    """
...
def get_dataset(dataset_name):
    ...
    if(dataset_name.lower() == "mnist"):        
        ...
        #data_train = GetMnist('train', shuffle=True, dir=dataset_loc) # has dictionary data_test._cache["train/test/extra"] that holds data   
        ...
        #data_train = get_cifar10_data('train', dir=dataset_loc) # has dictionary data_test._cache["train/test/extra"] that holds data   
...
def initiate_traning(userconfig):
    ...
    epochs = userconfig["training-options"]["epochs"]
    ...
    logger.set_logger_dir(os.path.join('logs','train_log', '{0}-{1}-{2}'.format(training_model, dataset_name, log_name)), action="d")    
    
    #get model architecture
    if(dataset_name == "mnist"):
        model = get_model(get_mod_typ(training_model, precision), precision)
    elif (dataset_name == "cifar10"):
        model = get_model_cifar(get_mod_typ_cifar(training_model, precision), precision)  
    else:
    ...
    if(dataset_name == "mnist"):
        ...
        config = get_config_mnist(model, data_train, data_test, epochs)  
    elif(dataset_name == "cifar10"):
        print("cifar selected")
        config = get_config_cifar(model, data_train, data_test, epochs)  
        if(SupportedModels.resnet_3 == get_mod_typ_cifar(training_model, precision) or 
            SupportedModels.resnet_5 ==  get_mod_typ_cifar(training_model, precision) or 
            SupportedModels.resnet_7 == get_mod_typ_cifar(training_model, precision) or
            SupportedModels.resnet_3q == get_mod_typ_cifar(training_model, precision) or
            SupportedModels.resnet_5q == get_mod_typ_cifar(training_model, precision) or
            SupportedModels.resnet_7q == get_mod_typ_cifar(training_model, precision)):
